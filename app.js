...
async function loadProducts() {
  try {
    // GitHub Pages: products.json na raiz é a fonte oficial da vitrine.
    // O parâmetro evita que o navegador reutilize uma versão antiga em cache.
    const staticRes = await fetch(`./products.json?v=${Date.now()}`, { cache: 'no-store' });

    if (!staticRes.ok) {
      throw new Error(`Não foi possível carregar products.json (${staticRes.status})`);
    }
